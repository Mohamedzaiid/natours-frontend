...
export function ModernChatResults({ results, loading, onAddToWishlist, isInPanel }) {
  ...
  if (loading) {
    return (
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-950/30 p-6 shadow-sm border border-emerald-100 dark:border-emerald-900/50">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-400 via-teal-500 to-emerald-400 animate-gradient"></div>
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center">
          <div className="relative mb-6">
            ...
            <div className="relative bg-emerald-100 dark:bg-emerald-900 p-4 rounded-full">
              <Loader2 size={24} className="text-emerald-600 dark:text-emerald-400 animate-spin" />
            </div>
          </div>
    ...
    return (
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/40 dark:to-orange-950/30 p-6 shadow-sm border border-amber-100 dark:border-amber-900/50">
        ...
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <button className="px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-full text-sm font-medium hover:from-amber-600 hover:to-orange-600 transition-colors shadow-md hover:shadow-lg">
            Create Custom Plan
          </button>
        </div>
      </div>
  ...
  return (
    <div className={cn(
      "relative overflow-hidden bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-950/30 shadow-sm border border-emerald-100 dark:border-emerald-900/50",
      isInPanel ? "" : "rounded-xl" 
    )}>
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-400 via-teal-500 to-emerald-400 animate-gradient"></div>
      ...
      <div className="p-4">
              ...
              index === activeIndex && (
                ...
                >
                  <div className="relative h-40 rounded-lg overflow-hidden mb-3">
                    {tour.imageCover ? (
                      <Image
                        src={`https://natours-yslc.onrender.com/img/tours/${tour.imageCover}`}
                        alt={tour.name}
                        fill
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center pt-2">
                      ...
                      <Link href={`/tour/${tour.slug || tour.id || tour._id}`} className="block">
                        <button className="px-3 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-full text-xs font-medium transition-colors shadow-sm hover:shadow-md flex items-center">
                          View Tour
        ...
        {results.length > 1 && (
          ...
          {isInPanel ? "Browse All Tours" : `View All ${results.length} Tours`}
          </button>
          </Link>
          </div>
        )}
...
export default ModernChatResults;
