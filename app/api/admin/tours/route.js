...
// GET handler to fetch all tours
export async function GET(request) {
  console.log('Admin API: GET all tours request received');
  ...
  console.log('Admin/lead-guide access verified, fetching tours');
  ...
  try {
    ...
    // Log response status
    console.log(`Backend tours API response status: ${response.status}`);
    
    // Handle error responses
    if (!response.ok) {
      return NextResponse.json(
        { status: 'error', message: 'Error fetching tours from backend' },
    ...
    const data = await response.json();
    console.log(`Successfully fetched ${data.results || 0} tours`);
    
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error in admin/tours GET route:', error);
    return NextResponse.json(
...
// POST handler to create a new tour
export async function POST(request) {
  console.log('Admin API: POST new tour request received');
  ...
  console.log('Admin/lead-guide access verified, creating tour');
  
  try {
    // Get FormData from request
    const formData = await request.formData();
    console.log('Tour creation data received as FormData');
    ...
    // Log response status
    console.log(`Backend tour creation API response status: ${response.status}`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          status: 'error', 
          message: errorData.message || 'Error creating tour' 
        },
        { status: response.status }
      );
    }
    ...
    // Parse and return successful response
    const data = await response.json();
    console.log(`Successfully created tour: ${data.data?.data?.name || 'New Tour'}`);
    
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error in admin/tours POST route:', error);
    return NextResponse.json(
      { status: 'error', message: `Server error: ${error.message}` },
      { status: 500 }
    );
