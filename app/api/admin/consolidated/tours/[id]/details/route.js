...
// GET handler for tour details with reviews and bookings
export async function GET(request, { params }) {
  const { id } = params;
  console.log(`Admin API: GET tour ${id} details request received`);
  ...
  console.log(`Admin access verified, fetching tour ${id} details`);
  ...
  try {
    ...
    const response = await fetch(`${ADMIN_API}/tours/${id}/details`, {
      method: 'GET',
      headers: {
        Cookie: cookieHeader || '',
    ...
    console.error(`Error in admin/consolidated/tours/${id}/details GET route:`, error);
    return NextResponse.json(
