...
// GET handler for user details with bookings and reviews
export async function GET(request, { params }) {
  const { id } = params;
  console.log(`Admin API: GET user ${id} details request received`);
  ...
  console.log(`Admin access verified, fetching user ${id} details`);
  ...
  try {
    ...
    const response = await fetch(`${ADMIN_API}/users/${id}/details`, {
      method: 'GET',
      headers: {
        Cookie: cookieHeader || '',
    ...
    console.error(`Error in admin/consolidated/users/${id}/details GET route:`, error);
    return NextResponse.json(
