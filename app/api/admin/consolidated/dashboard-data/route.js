...
export async function GET(request) {
  console.log('Admin API: GET consolidated dashboard data request received');
  ...
  console.log('Admin access verified, fetching consolidated dashboard data');
  
  // Get cookie for authentication
  const cookieHeader = request.headers.get('cookie');
  
  try {
    // Forward the request to our new consolidated backend API
    const response = await fetch(`${ADMIN_API}/dashboard-data`, {
      method: 'GET',
      headers: {
        Cookie: cookieHeader || '',
    ...
    console.error('Error in admin/consolidated/dashboard-data GET route:', error);
    return NextResponse.json(
